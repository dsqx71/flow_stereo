...
def block(img1, img2, flow, name, factor=1,out_dim=2,up_sample=True):
    ...
    flow_tmp = mx.sym.transpose(data=flow, axes=(0, 2, 3, 1), name=name + 'flow_transpose')
...
def spynet_symbol(type):
    ...
    for i in range(4):
        ...
        img1_downsample.append(mx.sym.Pooling(data=img1_tmp, kernel=(2,2),  stride=(2, 2), pool_type='avg'))
        img2_downsample.append(mx.sym.Pooling(data=img2_tmp, kernel=(2, 2), stride=(2, 2), pool_type='avg'))

    v0 = init(img1_downsample[3], img2_downsample[3], out_dim=out_dim)
    v1 = block(img1_downsample[2], img2_downsample[2], v0, name='v1', factor=1, out_dim=out_dim)
    v2 = block(img1_downsample[1], img2_downsample[1], v1, name = 'v2', factor=1, out_dim=out_dim)
    v3 = block(img1_downsample[0], img2_downsample[0], v2, name = 'v3', factor=1, out_dim=out_dim)
    v4 = block(img1, img2, v3, name = 'v4', factor=1, out_dim=out_dim)


    loss1 = mx.sym.MAERegressionOutput(data=v4, label=flow1/1, name='loss4', grad_scale=1.00)
    loss2 = mx.sym.MAERegressionOutput(data=v3, label=flow2/2, name='loss3', grad_scale=1.00)
    loss3 = mx.sym.MAERegressionOutput(data=v2, label=flow3/4, name='loss2', grad_scale=1.00)
    loss4 = mx.sym.MAERegressionOutput(data=v1, label=flow4/8, name='loss1', grad_scale=0.50)
    loss5 = mx.sym.MAERegressionOutput(data=v0, label=flow5/16, name='loss0', grad_scale=0.20)

    loss = mx.sym.Group([loss1, loss2, loss3, loss4, loss5])
