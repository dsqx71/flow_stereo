...
def dispnet_s(data, labels, loss_scale, net_type='flow', is_sparse = False, name='dispnet-s'):
...
def dispnet2CSS(loss_scale, net_type='flow', is_sparse=False):
    ...
    # dispnet-s1
    data = warp_dispnet(img1, img2, disp=dispnetc_prediction['loss1'], name='s1', factor=2)
    dispnets1_prediction, dispnets1_loss = dispnet_s(data, labels, loss_scale['dispnets1'],
                                                     net_type=net_type, is_sparse = is_sparse,
                                                     name='dispnet-s1')

    # dispnet-s2
    data = warp_dispnet(img1, img2, disp=dispnets1_prediction['loss1'], name='s2', factor=2)
    dispnets2_prediction, dispnets2_loss = dispnet_s(data, labels, loss_scale['dispnets2'],
                                                     net_type=net_type, is_sparse=is_sparse,
                                                     name='dispnet-s2')
    loss = dispnets2_loss
    loss.extend(dispnets1_loss)
    loss.extend(dispnetc_loss)
